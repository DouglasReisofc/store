...
<div class="payment-section ptb-80">
    <div class="container">
        ...
        <div class="row justify-content-center">
            @lang('Current Balance') - {{ __($general->cur_sym) }} {{ showAmount(Auth::user()->balance, 2) }}
            <div class="col-lg-6">
                <div class="select-item">
                    <select class="method form--control select-bar m-0">
                        <option value="">
                            @lang('Select Payment Gateway')
                        </option>
        ...
        <div class="modal-content">
            ...
            <form action="{{route('user.deposit.insert')}}" method="post">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label class="text--base">@lang('Insira o valor'):</label>
                        <div class="input-group">
                            <input id="amount" type="text" class="form-control form--control" name="amount" required value="{{old('amount')}}" inputmode="numeric">
...
<script>
    (function ($) {
        ...
        $('.deposit').on('click', function () {
            ...
            var depositLimit = `@lang('Limite de Pagamento'): ${minAmount} - ${maxAmount}  ${baseSymbol}`;
            $('.depositLimit').text(depositLimit);
            var depositCharge = `@lang('Charge'): ${fixCharge} ${baseSymbol}  ${(0 < percentCharge) ? ' + ' +percentCharge + ' % ' : ''}`;
            $('.depositCharge').text(depositCharge);
            $('.method-name').text(`@lang('Pagamento via ') ${name}`);
