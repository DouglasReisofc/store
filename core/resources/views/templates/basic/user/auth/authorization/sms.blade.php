...
<div class="account-section bg-img" style="background-image: url( {{ getImage('assets/images/frontend/authentication/' .@$bgImage->data_values->image, '1920x1080') }} );">
    <div class="left">
        <div class="left-inner w-100">
            ...
            <form class="account-form row g-4" id="smsForm" action="{{ route('user.verify.sms') }}" method="POST">
                @csrf
                <div class="col-md-12 text-center">
                    <label for="username" class="form--label">@lang('Telefone/whatsapp'):
                        <strong>{{ auth()->user()->mobile }}</strong>
                    </label>
                    <!-- Altere o botão para um link -->
                <div class="col-md-12">
                    <a href="#" id="changeMobileLink">@lang('Alterar')</a>
                </div>
                </div>
                ...
                <div class="col-md-12">
                    <button type="button" class="cmn--btn btn--lg w-100 justify-content-center" id="submitBtn">@lang('Submit')</button>
                </div>
                <div class="col-md-12">
                    @lang('Se você não receber nenhum código')
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="changeMobileModalLabel">@lang('Alterar número de Telefone')</h5>
            ...
            <div class="modal-body">
                ...
                <form action="{{ route('user.send.verify.mobile') }}" method="POST">
                    @csrf
                    <input type="hidden" name="type" value="phone">
                    <div class="col-md-12">
                        <label for="new_mobile_modal" class="form--label">@lang('Seu novo número')</label>
                        <input type="tel" required="required" maxlength="11" name="new_mobile" id="new_mobile_modal" class="form-control form--control" placeholder="@lang('(xx) 9xxxx-xxxx')" value="">
                        <small class="form-text text-muted">Digite apenas o DDD e o número, sem espaços ou caracteres especiais.</small>
                    </div>
                    <button type="submit" class="btn btn--base btn-block mt-3">@lang('Alterar')</button>
                </form>
            </div>
        </div>
    </div>
