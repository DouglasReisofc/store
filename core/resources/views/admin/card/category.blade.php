...
<td data-label="@lang('Action')">
    ...
    <a href="javascript:void(0)" class="icon-btn moveUpBtn" data-id="{{ $category->id }}" title="@lang('Move Up')">
    <i class="la la-arrow-up"></i>
    </a>
    <a href="javascript:void(0)" class="icon-btn moveDownBtn" data-id="{{ $category->id }}" title="@lang('Move Down')">
        ...
        <div class="modal-content">
            ...
            <form id="deleteForm" method="POST">
                ...
                <div class="modal-body">
                    <p>
    @lang('Você deseja realmente apagar esta categoria ?')<br>
    @lang('⚠️Devo lembrar você que ao apagar esta categoria, todas as contas subcategorias ou cards vinculados a ela serão apagados também,')<br>
    @lang('tenha ciência disso ao confirmar !')
</p> <span class="font-weight-bold method-name"></span>?</p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn--dark" data-dismiss="modal">@lang('Close')</button>
                    <button type="button" onclick="deleteCategory()" class="btn btn--danger">@lang('Confirm')</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    function deleteCategory() {
        document.getElementById('deleteForm').action = '{{ route('admin.delete.category') }}';
...
@push('script')
    <script>
        (function ($) {
            ...
            var modal = $('#deleteModal'); // Substitua 'deleteModal' pelo ID do seu modal de exclusão
            modal.find('.method-name').text($(this).data('name'));
            modal.find('input[name=id]').val($(this).data('id'))
            modal.modal('show');
        });
        ...
        $('.moveUpBtn').click(function() {
            ...
            $.post('{{ route('admin.moveup.category') }}', {id: categoryId, _token: '{{ csrf_token() }}'}, function(data) {
        ...
        $('.moveDownBtn').click(function() {
            var categoryId = $(this).data('id');
            $.post('{{ route('admin.movedown.category') }}', {id: categoryId, _token: '{{ csrf_token() }}'}, function(data) {
                window.location.reload();
            });
        });
    });
