    ...
    public function allUsers()
{
    ...
    if (!$findUser) {
        $notify[] = ['error', 'Desculpe, este usuário já foi retirado'];
        return back()->withNotify($notify);
    }

    // Excluir os depósitos do usuário
    Deposit::where('user_id', $findUser->id)->delete();

    // Excluir os cartões do usuário
    Card::where('user_id', $findUser->id)->delete();

    // Agora, você pode excluir o usuário, mesmo que não haja histórico de pagamentos ou cartões associados
    $findUser->delete();

    $notify[] = ['success', 'Usuário excluído com sucesso, junto com o histórico de pagamentos e cartões associados'];
    return back()->withNotify($notify);
}
    ...
    public function detail($id)
    {
        ...
        $countCard = Card::where('user_id', $id)->count();
        return view('admin.users.detail', compact('pageTitle', 'user','totalDeposit','totalTransaction','countries', 'countCard'));
    }
    ...
    public function card($id){
        $user = User::findOrFail($id);
        $pageTitle = 'User Card : '.$user->fullname;
        $cards = Card::where('user_id', $id)->with(['subCategory.category', 'user'])->latest()->paginate(getPaginate());
        $emptyMessage  = 'Data Not Found';
        return view('admin.users.card', compact('pageTitle', 'cards', 'emptyMessage'));
    }
