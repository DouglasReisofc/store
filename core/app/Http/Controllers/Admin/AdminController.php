    ...
    public function profileUpdate(Request $request)
{
    $this->validate($request, [
        ...
        'mobile' => 'required', // Adicione esta linha para validar o campo mobile
    ...
    $user->mobile = $request->mobile; // Adicione esta linha para salvar o campo mobile
    $user->save();

    $notify[] = ['success', 'Seu perfil foi atualizado'];
    ...
    public function topSold (){
        ...
        $cards = Card::where('user_id', '!=', 0)
                    ->selectRaw('sub_category_id, count(sub_category_id) as sold')
    ...
    public function todaySale(){
        ...
        $cards = Card::where('user_id', '!=', 0)
                    ->selectRaw('sub_category_id, count(sub_category_id) as sold')
                    ->whereDate('purchase_at', Carbon::today())
                    ->groupBy('sub_category_id')
                    ->orderBy('sold', 'DESC')
                    ->with('subCategory.category')
                    ->paginate(getPaginate());
