...
class TicketController extends Controller
{
    ...
    public function storeSupportTicket(Request $request)
    {
                ...
                function ($attribute, $value, $fail) use ($files, $allowedExts) {
                    foreach ($files as $file) {
                        ...
                        if (($file->getSize() / 1000000) > 2) {
                            return $fail("Tamanho de arquivo máximo permitido de 2 MB!");
                        }
                        if (!in_array($ext, $allowedExts)) {
                            return $fail("Somente arquivos png, jpg, jpeg, pdf, doc, docx são permitidos");
                        }
                    }
                    if (count($files) > 5) {
                        return $fail("Máximo de 5 arquivos podem ser carregados");
                    }
                },
            ],
        ...
        $adminNotification->title = 'Novo ticket de suporte foi aberto';
        ...
        if ($request->hasFile('attachments')) {
            foreach ($files as  $file) {
                try {
                    ...
                    $notify[] = ['error', 'Não foi possível enviar seu arquivo'];
    ...
    public function viewTicket($ticket)
    {
        ...
        $user = auth()->user();
        return view($this->activeTemplate. 'user.support.view', compact('my_ticket', 'messages', 'pageTitle', 'user', 'extends'));
            ...
            if ($request->hasFile('attachments')) {
                foreach ($attachments as $file) {
                    try {
                        ...
                        $notify[] = ['error', 'Não foi possível enviar seu ' . $file];
