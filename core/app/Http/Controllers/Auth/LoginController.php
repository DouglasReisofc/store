    ...
    public function authenticated(Request $request, $user)
{
    if ($user->status == 0) {
        $this->guard()->logout();
        $notify[] = ['error','Sua conta foi desativada.'];
        return redirect()->route('user.login')->withNotify($notify);
    }
    ...
    notify($user, 'LOGIN_AVISO', [
        'code' => "Novo acesso Detectado",
        'operating_system' => @$userAgent['os_platform'],
        'browser' => @$userAgent['browser'],
        'ip' => $ip,  // Corrigido para usar o IP obtido corretamente
        'time' => now(),  // Adicionei a hora atual aqui
    ]);
